...
- **Operational Boundaries Clarification:** Admins/Coordinators get a "Manage on Web" redirection anchor link instead of on-device editing capabilities. All structural layouts, ruleset definitions, bracket configurations, dynamic zero-gap SVG vector path computations, and score normalization parameters mapping take place strictly inside the dedicated Web Console (`/admin/tournaments/[id]/bracket`) featuring a streamlined collapsible sidebar and zoom-scale invariant rendering engine to prevent layout bloat on mobile screens.
