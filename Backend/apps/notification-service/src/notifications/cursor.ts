...
/**
 * Keyset pagination for the inbox. Every list here is ordered by `created_at` descending with
 * `_id` breaking ties, so this is the same single-field helper as
 * `points-service/src/points/cursor.ts` (and, before it, `announcement-service/audience.ts`).
 *
 * Never skip/offset: the inbox only grows at the head, so an offset page walks further every time
 * and a notification that arrives mid-pagination shifts every later page by one — the reader sees
 * a row twice and never sees the one it displaced.
 */
 ...
 * first page) or the `user_id` filter (one user reading another's inbox).
 */
