...
| [x]    | API Gateway (Kong / Express Gateway) | – Route requests to services <br> – JWT verification <br> – Rate limiting (5 auth attempts/15min, 100 req/min for general) | 2 | Implemented as root NestJS app with http-proxy-middleware. Routes all 7 services. | Dhruvin |
| [x]    | Event Bus (MVP: in‑memory) | – Simple `EventEmitter` based bus <br> – Define domain events (`UserRegistered`, `EventCreated`) | 1 | In-memory EventBusService stub in all 4 event-emitting services. Kafka deferred to Phase 2. | Dhruvin |

### Milestone 0.3 – Database & Cache

| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | PostgreSQL schema | – Run migrations for `users`, `events`, `registrations`, `sponsors`, `points_transactions`, `announcements` | 2 | All migrations present across all 7 services. | Dhruvin |
| [x]    | Redis cache layer | – Cache user sessions (refresh tokens) <br> – Cache event list (5 min TTL) | 1 | Redis used for sessions and rate limiting. Event list TTL caching deferred (no significant load yet). | Dhruvin |
| [x]    | Basic seeding | – Script to insert dummy users, sponsors, events | 0.5 | `backend/src/seed.ts` — idempotent seed with users, sponsors, affiliations, events, registrations, scores, transactions, announcements. | Dhruvin |

### Milestone 0.4 – Frontend Shell (Mobile + Web)

| Status | Task                    | Sub‑tasks                                                                                                 | Est. | Done By |
|--------|-------------------------|-----------------------------------------------------------------------------------------------------------|------|---------|
| [x]    | React Native (Expo) app | – Navigation drawer (Side Drawer) <br> – Dynamic status bar component <br> – Theme switching (dark/light) | 3    | `mobile/src/components/drawer-content.tsx`, `dynamic-status-bar.tsx`, `themeStore.ts`. Full drawer shell with 12 routes. | Satyam |
| [x]    | React Web (Admin PWA)   | – Tailwind CSS + Vite <br> – Router (login, basic event table)                                            | 2    | `web/src/app/router.tsx` — `/login`, `/auth/callback`, `/events` (EventTable wired to EventRepository). | Satyam |
| [x]    | MVVM base classes       | – `BaseViewModel` with observable state <br> – Repository pattern for API calls                           | 2    | `BaseViewModel.ts`, `useViewModel.ts`, `ApiClient.ts` in both `mobile/src/core` and `web/src/core`. | Satyam |
...
| Status | Task                         | Sub‑tasks                                                                                                                          | Est. | Done By |
|--------|------------------------------|------------------------------------------------------------------------------------------------------------------------------------|------|---------|
| [x]    | Sponsor service              | – CRUD for `sponsors` (name, logo, tenure_start/end) <br> – `UserSponsorAffiliation` table <br> – Endpoint: `GET /sponsors/active` | 2    | sponsor-service port 3003, full CRUD + active filtering + soft delete. | Dhruvin |
| [x]    | Onboarding sponsor selection | – `POST /users/me/sponsor` (with semester change limit) <br> – Validate that user can change only once per semester                | 1    | In user-service. Semester logic + affiliation row creation. | Dhruvin |
| [x]    | Basic fan counting           | – Emit `FanEarned` event when user wins an event (stub for now)                                                                    | 1    | `POST /sponsors/:id/fans` awards fans, emits FanEarned domain event, recalculates total_fans. | Dhruvin |
| [x]    | Sponsor leaderboard          | – `GET /sponsors/leaderboard?sort=fans` – aggregate fan counts                                                                     | 1    | Supports `?sort=fans|events|users`. Live aggregation from affiliations. | Dhruvin |

#### ✅ Milestone 1.2 – Events & Registration (Weeks 2‑3)

| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | Event service | – `POST /events` (admin only) <br> – `GET /events` (with filters: upcoming/ongoing/past) <br> – `GET /events/:id` | 3 | event-service port 3004. Full CRUD + status filter + hall-of-fame module. | Dhruvin |
| [x]    | Registration flow | – `POST /events/:id/register` (solo only, no teams in MVP) <br> – Check registration deadline, capacity <br> – Emit `RegistrationCreated` event | 2 | Capacity + deadline + duplicate checks. `RegistrationCreated` emitted in-memory. | Dhruvin |
| [x]    | Points service (basic) | – Consume `RegistrationCreated` → award participation points <br> – Emit `PointsEarned` | 2 | points-service port 3005. Manual `POST /points/participation` bridge until Kafka. Emits `PointsEarned`. | Dhruvin |
| [x]    | Event leaderboard (MVP) | – For `LE` events: admin can manually enter scores via `POST /events/:id/scores` <br> – `GET /events/:id/leaderboard` | 2 | Score submission is transactional (delete+validate+insert). Leaderboard ranked by score desc. | Dhruvin |
| [x]    | Post‑event fan award | – When `EventCompleted` is emitted, award fans to winners’ sponsors | 1 | `PATCH /events/:id/complete` emits `EventCompleted` and calls `POST /sponsors/:id/fans` per winner via HTTP. | Dhruvin |

#### ✅ Milestone 1.3 – User Profile & Player Card (Week 4)

| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | Profile API | – `GET /users/:id` (public fields) <br> – `PATCH /users/me` (bio, interests, social links) | 2 | `GET /users/:id` returns public profile. `PATCH /users/me` + `PATCH /users/me/profile` for extended fields. | Dhruvin |
| [x]    | Player card generation | – Endpoint that returns a JSON with avatar, username, sponsor badge, stats | 1 | `GET /users/me/player-card` — aggregates sponsor name/logo via HTTP to sponsor-service. | Dhruvin |
| [x]    | Interest fields | – `GET /interests` (list of sports/esports) <br> – `PATCH /users/me/interests` | 1 | Static catalog + DB-stored user interests. | Dhruvin |
| [x]    | Sponsor stats on profile | – `GET /users/me/sponsor-stats` (personal fan count, events won) | 1 | Returns fan_count, events_won[], total_points_contributed from affiliation row. | Dhruvin |

#### ✅ Milestone 1.4 – Announcements & Admin Panel (Week 5)

| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | Announcement service | – `POST /announcements` (coordinator+) <br> – `GET /announcements` (public, last 4 months) <br> – Announcement types & tags | 2 | announcement-service port 3007. Added `PATCH /:id`, `GET /:id`, `?tag=` filter. 9 announcement types. | Dhruvin |
| [x]    | Basic users table (admin) | – `GET /admin/users` (paginated, filter by role/status) – coordinator+ | 2 | `GET /users?role=&status=&page=&limit=` — coordinator+ only. Pagination implemented. | Dhruvin |
| [x]    | Sponsor management (admin) | – `POST /admin/sponsors` <br> – `PATCH /admin/sponsors/:id/tenure-end` (manual for MVP) | 1 | `POST /sponsors`, `PATCH /sponsors/:id`, `PATCH /sponsors/:id/tenure-end` all present in sponsor-service. | Dhruvin |

#### ✅ Milestone 1.5 – Points & Hall of Fame v1 (Week 6)

| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | Points balance | – `GET /users/me/points` <br> – Transaction history (`GET /points/transactions`) | 1 | `GET /points/me/balance`, `GET /points/me/transactions?source=&type=&page=&limit=`, `GET /points/transactions/:userId` (admin). | Dhruvin |
| [x]    | Hall of Fame | – `GET /hall-of-fame/event-winners` <br> – `GET /hall-of-fame/sponsor-champions` (hardcoded or manually seeded) | 2 | HallOfFameModule inside event-service. event-winners from DB, sponsor-champions from sponsor-service HTTP. | Dhruvin |
| [x]    | Notification service (in‑app) | – Store notifications in DB <br> – `GET /notifications` (unread count) <br> – `PATCH /notifications/:id/read` | 2 | notification-service port 3006. `GET /notifications/me` returns `{notifications,unreadCount,total}`. Added `PATCH /notifications/read-all`. | Dhruvin |

**Final Backend MVP Checklist (June 26)**
- [x] All endpoints above are implemented, documented (Swagger), and tested.
- [x] The event bus (in‑memory) passes events between services correctly.
- [x] PostgreSQL schema is fully migrated; Redis caches session and event lists.
- [ ] Postman/Newman collection runs without error.
- [ ] Backend deployed to staging environment (Railway/Render).
- [x] Security: JWT expires after 15 min, refresh token rotation works.
...
| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | Auth screens | – Login / Register (email + Google OAuth) <br> – Sponsor selection during onboarding <br> – Interest selection | 2 | `login.tsx`, `register.tsx`, `auth/complete-profile.tsx`, `auth/otp.tsx`, `auth/callback.tsx`. Auth wired to real auth-service via `AuthRepository`. | Satyam |
| [x]    | Home page | – Landing intro (static) <br> – Announcements list (from API) <br> – Public social feed (read‑only) | 2 | `(drawer)/index.tsx` — 3-tab layout: IntroTab, AnnouncementsTab (real API via `AnnouncementRepository`), FeedTab (mock data, wired to `use-feed` — real social-service integration is Phase 2). | Satyam |
| [x]    | Events page | – Browse events (upcoming/ongoing/past) <br> – Solo registration flow <br> – Event details view | 2 | `(drawer)/events.tsx` + `event/[id].tsx`. Wired to `EventRepository` (real API). Registration, leaderboard, bracket sections all present. | Satyam |
| [x]    | User profile | – View own profile (player card, sponsor badge) <br> – Edit bio/interests | 1 | `(drawer)/profile.tsx` — `PlayerCard`, `UserInfoCard`, `HistorySection`, `SettingsSection`. Wired to `use-profile`. | Satyam |
| [x]    | Points & Hall of Fame | – Display points balance <br> – Hall of Fame screen (winners + sponsor champions) | 1 | `(drawer)/points.tsx` wired to `PointsRepository`. `(drawer)/hall-of-fame.tsx` wired to `HallOfFameRepository` (real API). | Satyam |

#### ✅ Milestone 1.7 – Web Admin Console (React + Tailwind)

| Status | Task | Sub‑tasks | Est. | Done By |
|--------|------|-----------|------|---------|
| [x]    | Login & RBAC | – Coordinator/Founder login redirects to admin dashboard | 0.5 | `Login.tsx` — email+password + Google OAuth, wired to real `AuthRepository`. `RequireAuth` guard on all admin routes. | Satyam |
| [x]    | Event management | – Create/edit events (form with title, dates, type, etc.) | 1 | `Events.tsx` + `EventTable` component wired to `EventRepository` (live API). | Satyam |
| [x]    | Announcement creator | – Make Announcement Popup (rich text, tag selection) | 1 | `BroadcastsPage.tsx` — full broadcast/announcement form (title, category, audience, content, push toggle, schedule). Wired to `AdminContext`. Note: routed via `AdminContext.currentTab = ‘broadcasts’`, not via `router.tsx`. | Satyam |
| [x]    | Users table | – View users, filter by role/sponsor | 1 | `DashboardPage.tsx` includes users/captains table. Full admin dashboard with `CaptainApplication`, `Ticket`, `ModerationItem` views. Routed via `AdminContext`. | Satyam |
...
|--------|------|---------|------|---------|
| [x]    | Friend request API | Send/accept/reject/block, list friends, mutual friends | 3 days | social-service port 3008. `POST /social/friends/requests`, `PATCH /social/friends/requests/:id/accept`, `PATCH /social/friends/requests/:id/reject`, `POST /social/friends/:userId/block`, `DELETE /social/friends/:userId`. | Dhruvin |
| [x]    | Friend suggestions | Based on mutual friends (second-degree connections) | 2 days | `GET /social/friends/suggestions` — ranks by mutual friend count. | Dhruvin |
| [ ]    | Friends UI (mobile) | Friend list tab, request inbox, add friend button | 3 days | — |
| [x]    | Online status | Redis‑based presence tracking (last active, online/offline) | 2 days | `POST /social/friends/presence` (5-min TTL heartbeat), `GET /social/friends/online`. | Dhruvin |

### ✅ Milestone 2.2 – Social Feed v2 (2 weeks)

| Status | Task | Details | Est. | Done By |
|--------|------|---------|------|---------|
| [x]    | Post creation | Caption, media URLs, tags, visibility (public/protected/private), likes/comments toggles | 3 days | `POST /social/posts`. media_urls are URL-based for MVP; full media pipeline in Phase 4. | Dhruvin |
| [x]    | Like & comment | Like/unlike, create comment, delete own comment | 2 days | `POST /social/posts/:id/like`, `DELETE /social/posts/:id/like`, `POST /social/posts/:id/comments`, `DELETE /social/posts/:postId/comments/:commentId`. | Dhruvin |
| [x]    | Feed aggregation | `GET /social/feed` – posts from friends + public posts, paginated | 2 days | Authenticated feed + `GET /social/feed/public` for guest view. | Dhruvin |
| [x]    | Privacy enforcement | Backend filters posts based on visibility & friendship | 2 days | public=anyone, protected=authenticated, private=friends only. Enforced in PostsService. | Dhruvin |

### ✅ Milestone 2.3 – Challenges & Leaderboards v2 (2 weeks)

| Status | Task | Details | Est. | Done By |
|--------|------|---------|------|---------|
| [x]    | Challenge service | CRUD for challenges (title, domain, difficulty, award points) | 2 days | challenge-service port 3009. Full CRUD with domain/difficulty/status filters. `GET /challenges`, `POST /challenges` (core+), `PATCH /challenges/:id`, `DELETE /challenges/:id`. | Dhruvin |
| [x]    | Accept & submit | `POST /challenges/:id/accept`, `POST /challenges/:id/submit` (with proof URL + text) | 2 days | ChallengeAcceptance + ChallengeSubmission entities. Must accept before submitting. | Dhruvin |
| [x]    | Points award on completion | Core+ reviews submission → on approval, HTTP bridge to points-service `/points/award` with `source: 'challenge'` | 1 day | `PATCH /challenges/:challengeId/submissions/:submissionId/review`. Fire-and-forget on approval. | Dhruvin |
| [ ]    | Auto‑leaderboards for LE events | Scores entered by admin → leaderboard auto‑updates via WebSockets | 3 days | Deferred to Phase 3 (requires WebSocket/Socket.io infrastructure). |
...
**Known gaps for Phase 2 frontend wiring (mobile):**
- `FriendRepository` and `ChallengeRepository` still serve mock data — need to be swapped to social-service and challenge-service endpoints now that the backend is live.
- `use-feed` serves `MOCK_FEED_POSTS` — needs wiring to `GET /social/feed`.
- No drawer-level challenge browse screen (`(drawer)/challenges.tsx`) — only a deep-link detail view exists.
